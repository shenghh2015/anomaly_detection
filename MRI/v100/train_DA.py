...
DA_model_name = 'cnn-{0:}-fc-{1:}-bn-{2:}-bz-{3:}-D_lr-{4:}-G_lr-{5:}-nD-{6:}-nG-{7:}-iter-{8:}-ac1-{9:}-ac2-{10:}'.format(dis_cnn, dis_fc, dis_bn, batch_size, d_lr, g_lr, nd_steps, ng_steps, nb_steps, dAcc1, dAcc2)
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		print_yellow('Loss: D {0:.4f}, G :{1:.4f}; Dom acc: {2:.4f} Iter:{3}'.format(D_loss/2, G_loss, domain_acc, iteration))
		# save results
