...
def plot_AUC2(file_name, auc_list, mse_auc_list):
	from matplotlib.backends.backend_agg import FigureCanvasAgg
	from matplotlib.figure import Figure
	fig_size = (8,6)
	fig = Figure(figsize=fig_size)
...
## model folder
model_name = 'FDA{}-{}-{}-cn-{}-fr-{}-ks-{}-bn-{}-skp-{}-res-{}-lr-{}-stps-{}-bz-{}-tr-{}k-vl-{}-test-{}-l-{}'.format(version, dataset, os.path.basename(output_folder), nb_cnn, filters, kernel_size, batch_norm, skip, residual, lr, nb_steps, batch_size, int(train/1000), val, test, loss)
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		if iteration%100 == 0:
			...
			if mse_display:
				tst_pixel_errs = np.square(Xt - y_recon)
				tst_img_errs = np.squeeze(np.apply_over_axes(np.mean, tst_pixel_errs, axes = [1,2,3]))
				mse_auc = roc_auc_score(yt, tst_img_errs)
				mse_auc_list.append(mse_auc)
				print('AUC: MSE {0:.4f}'.format(mse_auc))
			...
			auc_file = os.path.join(model_folder,'auc-{}.png'.format(model_name)); plot_AUC2(auc_file, auc_list, mse_auc_list)
