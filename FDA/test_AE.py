...
model_name = 'FDA2-dense-FDA-cn-4-fr-32-ks-3-bn-True-skp-False-res-False-lr-1e-05-stps-300000-bz-50-tr-7k-vl-200-test-200-l-ssim'
splits = model_name.split('-')
if len(splits[0])<=2:
	version =1
else:
	version = int(splits[0][3])
...
# 		anomaly_errs = err_correntropy.eval(session = sess, feed_dict = {x: X_SP_tst})
		...
		print_green('Anomaly detection auc: Correntropy {0:.4f}'.format(detect_auc))
		# ssim
		tst_ssim = ssim_err.eval(session = sess, feed_dict = {x:X_SA_tst})
		anomaly_ssim = ssim_err.eval(session = sess, feed_dict = {x: X_SP_tst})
		recon_errs = np.concatenate([tst_ssim, anomaly_ssim])
		detect_auc = roc_auc_score(yt, recon_errs)
		print_green('Anomaly detection auc: SSIM {0:.4f}'.format(detect_auc))		
